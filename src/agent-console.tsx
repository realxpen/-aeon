...
export default function AgentConsole(){
 ...
 useEffect(()=>{
   ...
   const decision=(event:Event)=>{
     ...
     const trace:WebMCPTrace={id:`decision_${Date.now()}`,tool:'human_authority',phase:'DECISION',input:detail,output:{decision:isApproved?'APPROVED':'DECLINED'},timestamp:Date.now()}
 ...
 const currentTool=approved?'purchase_execution':current?.tool??'create_mission'
 ...
 const status=approved?'APPROVED':current?.phase==='ERROR'?'ERROR':current?'ACTIVE':'READY'
 const capabilityMessage=approved?'Purchase execution released after human approval.':'Waiting for agent activity'
