...
export default function NoDealPanel({budget,candidates,onIncreaseBudget,onChangeRules,onRetry,onEnd}:Props){
 ...
 return <div className="no-deal panel">
  <div className="empty-hero" aria-hidden="true">
   <div className="empty-bot">AE</div>
   <span className="empty-spark spark-one">+</span><span className="empty-spark spark-two">·</span><span className="empty-spark spark-three">+</span>
  </div>
  ...
  <p className="no-deal-lead">I searched the market, checked mission fit, and tested the budget. Nothing I found can be proposed without breaking your <strong>₦{budget.toLocaleString()}</strong> spending ceiling.</p>
  <div className="empty-explanation"><span className="empty-status-dot"/><div><strong>AEON stopped itself.</strong><span>Your authority boundary remains intact. No non-compliant purchase was attempted.</span></div></div>
