...
export function getMissionConstitution():MissionConstitution{try{const raw=localStorage.getItem(KEY);if(!raw)return defaults;const parsed=JSON.parse(raw);const isLegacyDefault=parsed?.budget===1000000&&parsed?.goal==='Build the best creator setup under ₦1,000,000.';const saved={...defaults,...parsed,...(isLegacyDefault?{goal:defaults.goal,budget:0}: {})};return {...saved,priorities:deriveMissionPriorities(saved.goal)}}catch{return defaults}}
