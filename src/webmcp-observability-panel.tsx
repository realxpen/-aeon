...
function resultCount(t:WebMCPTrace){
  ...
  if(t.tool==='search_products'){
    if(Array.isArray(out))return out.reduce((n,r)=>n+(Array.isArray(r?.products)?r.products.length:0),0)
    if(Array.isArray(out?.products))return out.products.length
    return 0
  }
  if(t.tool==='compare_products'){
    if(Array.isArray(out))return out.length
    if(Array.isArray(out?.ranked))return out.ranked.length
    return 0
  }
  return undefined
}
...
function detail(t:WebMCPTrace){
  if(t.phase==='DECISION'){
    const d=(t.output as any)?.decision
    return d==='APPROVED'?'Human approved the proposed purchase. Execution may proceed.':'Human declined the proposed purchase. Execution remains locked.'
  }
  if(t.phase==='CALL'){
    const input=t.input as any
    if(t.tool==='search_products'&&input?.query)return `Searching the marketplace for “${input.query}”${input.maxPrice?` · ceiling ₦${Number(input.maxPrice).toLocaleString()}`:''}`
    if(t.tool==='compare_products')return `Comparing ${Array.isArray(input?.productIds)?input.productIds.length:0} marketplace candidates against the mission constraints`
    if(t.tool==='negotiate_offer')return `Opening seller-agent negotiation for ${input?.product??'the selected product'}${input?.listedPrice?` · listed ₦${Number(input.listedPrice).toLocaleString()}`:''}`
    return `${label(t.tool)} capability invoked for this mission`
  }
  const out=t.output as any
  if(t.phase==='ERROR')return out?.error??'Capability failed'
  if(t.tool==='search_products'){
    const n=resultCount(t)??0
    return `${n} marketplace candidate${n===1?'':'s'} found within the active mission ceiling`
  }
  if(t.tool==='compare_products'){
    const n=resultCount(t)??0
    return `${n} candidate${n===1?'':'s'} ranked for the active mission`
  }
  if(t.tool==='negotiate_offer'){
    if(out?.acceptedPrice!==undefined)return `Seller offer returned: ₦${Number(out.acceptedPrice).toLocaleString()}${out.saving>0?` · saved ₦${Number(out.saving).toLocaleString()}`:''}`
    return 'Seller negotiation completed for this mission'
  }
