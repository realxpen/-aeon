...
export default function Mission(){
  ...
  const [inputIssue,setInputIssue]=useState<'invalid'|'unavailable'|''>('')
  ...
  return <main className={`mission ${missionStarted?'mission-running':''}`}>
    ...
    <section className={`mission-input panel ${missionStarted?'mission-input-active':''}`}>
      ...
      <textarea id="goal" value={goal} onChange={e=>{setGoal(e.target.value);if(inputIssue)setInputIssue('')}} placeholder="e.g. Find me a phone under ₦500,000 and negotiate the best deal" rows={3}/>
