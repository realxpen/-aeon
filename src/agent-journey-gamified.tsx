...
export default function AgentJourneyGamified({ phase, running, approved, products, proposal, budget = 0 }: Props) {
  ...
  const negotiationThreads = useMemo(() => currentBasket?.products.map((product,index) => ({ product, deal: currentBasket.deals[index] })).filter(item => Boolean(item.deal?.transcript?.length)) ?? [], [currentBasket])

  // The journey follows explicit mission signals. There is deliberately no window scrolling here.
  // The six-card stack remains the visual focus; WebMCP/Firewall are observability, never focus targets.
  useEffect(() => {
    if (declined || approved) return
    if (open === 2) return
    if (target > open) setOpen(Math.min(open + 1, target))
  }, [target, open, approved, declined])

  // Negotiation owns the transition out of NEGOTIATE. The stage cannot advance until every
  // transcript message for every seller has been rendered and the final transcript is reached.
  useEffect(() => {
    if (open !== 2 || negotiationDone || negotiationThreads.length === 0) return
    const thread = negotiationThreads[negotiationProduct]
    if (!thread) { setNegotiationDone(true); return }
    if (negotiationMessage < thread.deal.transcript.length - 1) {
      const timer = window.setTimeout(() => { setNegotiationMessage(value => value + 1); playTick() }, 1800)
      return () => window.clearTimeout(timer)
    }
    if (negotiationProduct < negotiationThreads.length - 1) {
      const timer = window.setTimeout(() => { setNegotiationProduct(value => value + 1); setNegotiationMessage(-1); playTick() }, 1200)
      return () => window.clearTimeout(timer)
    }
    setNegotiationDone(true)
  }, [open, negotiationProduct, negotiationMessage, negotiationThreads, negotiationDone])

  // A completed negotiation is the only local signal that permits NEGOTIATE -> GOVERN.
  useEffect(() => {
    if (open === 2 && negotiationDone && !approved && !declined) setOpen(3)
  }, [open, negotiationDone, approved, declined])

  // GOVERN -> APPROVAL is controlled by the real mission phase, not a presentation timer.
  useEffect(() => {
    if (declined || approved || open !== 3) return
    if (phase === 'HUMAN APPROVAL' || target >= 4) setOpen(4)
  }, [phase, target, open, approved, declined])
  ...
  const replaceProduct = (oldId: string, next: Product) => {
    ...
    setWorking({ products: ps, deals: ds, total: ds.reduce((s,d)=>s+d.acceptedPrice,0), saving: ds.reduce((s,d)=>s+d.saving,0) }); playTick()
  }
  ...
  const openProduct = (product: Product) => {
    ...
    setDetail({ product, deal: index >= 0 ? currentBasket?.deals[index] : undefined }); playTick()
  }
  ...
  const visibleTranscript = negotiationThreads[negotiationProduct]?.deal.transcript.slice(0, Math.max(0, negotiationMessage + 1)) ?? []
    ...
    <section className="gamified-journey" aria-live="polite">
      ...
      <div className="stage-stack" role="list">
        {stages.map(([phaseName, label], index) => {
          ...
          return <JourneyCard key={phaseName} index={index} title={label} state={done ? 'done' : index === open ? 'active' : 'ready'} current={current} depth={depth} onOpen={() => setOpen(index)}>
